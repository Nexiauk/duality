...
![Am I responsive](./docs/screenshots/amiresponsive.jpg)
...
**Loop:** Discover → Register → Browse → Filter → Purchase → Own → Collect → Repeat
...
![Lunch Atop Skyscraper colour palette](./docs/screenshots/colour-palette.png)
...
![EightShapes Contrast Grid](./docs/screenshots/contrast-grid.png)
...
![Original Duality image](./docs/screenshots/original-duality.png)  
...
![Final Duality Image](./docs/screenshots/hero-image.png)
...
![Infinity flame icon](./docs/screenshots/original-logo.png)  
The colours weren't quite right so again, I used Adobe Photoshop to change it up:  
![Final Duality Logo](./docs/screenshots/logo.avif)
...
![Font Pairing](./docs/screenshots/font-pairing.png)
...
I didn't end up creating the fancy progress bar that causes the website background to change colour based on quantity of purchased cards by alignment - that has been earmarked as a future feature.
...
Using ChatGPT and Gemini, we created mapping logic to assign one of the 12 [Hero's Journey archetypes](https://screencraft.org/blog/breaking-down-the-character-archetypes-of-the-heros-journey/#:~:text=The%20Ordinary%20World:%20We%20see,take%20on%20the%20biggest%20conflict.) to every character in the file. 
...
* The image URLs are pulled from the legends.json file - these images come from a CDN so their quality varies depending on the original source. There are multiple image sizes but again, they vary so much in quality that the impact on performance due to the larger file sizes wasn't worth it so I chose the medium size as it seems to perform and look the best **for the most part**. As I am noticing characters with poor quality images, I am removing them from rotation so they don't appear in the shop.
...
![Shop card](./docs/screenshots/shop-card.jpg)
![Shop card hover effect](./docs/screenshots/shop-card-hover.jpg)

In the binder, cards flip on hover for additional content.  
![Binder card](./docs/screenshots/binder-card.jpg)
![Binder card hover effect](./docs/screenshots/binder-card-hover.jpg)
...
### *Footer*
The footer features a copyright notice, a socials icon area and shows whether a user is signed in or out. The footer is sticky but only on screens 768px and up,  not on mobile, to make more room for the content.  
...
![Socials icons hover effect](./docs/screenshots/socials-hover.jpg)
...
![Login status area](./docs/screenshots/login-status.jpg)
...
![Home page on a larger screen](./docs/screenshots/hero-section-lg.jpg)
...
![Home page on a mobile screen](./docs/screenshots/hero-section-mobile.jpg)
...
![Shop page](./docs/screenshots/shop.jpg)
...
![Shop filters](./docs/screenshots/filters.jpg)
...
![Shop filters](./docs/screenshots/filter-hover.jpg)  
![Shop filters](./docs/screenshots/filter-options.jpg)
...
![Card that is already owned](./docs/screenshots/card-owned.jpg)  
![Card with buy button](./docs/screenshots/card-price.jpg)
![Card displayed for guest](./docs/screenshots/shop-card.jpg)
...
![Confirmation of purchase page](./docs/screenshots/continue%20purchase.jpg)
...
![Stripe character card details](./docs/screenshots/stripe-card.jpg)
...
*Note:* Logic has been built into the system so if two users happen to access the shop at the same time, and there is no current schedule, it won't crash trying to create two schedules when there can only be one. Instead, it will allow a single schedule to be created and will retry fetching that schedule for the other request. This is handled using a constraint on the Scheduler start time, and try/except on a class method in the scheduler model that either gets or creates an active schedule.

### *Binder Page*
The binder page features filters, much the same as the shop page - by Rarity, Alignment and Universe. Rarity is all existing rarities on the model, while alignment and universe are collated directly from the usercards themselves. The Binder will show usercards if a user has any, and if not, text will be displayed stating the binder is empty with a button that kinks to the shop.        
![Empty binder page](./docs/screenshots/empty-binder.jpg)

As per the [Card Features](#cards) section, the cards also flip on hover to provide extended information about each character.

![Binder page](./docs/screenshots/binder.jpg)  
...
### *Admin*
* Shop Scheduler admin interface is sorted by Schedule start time descending, so the latest schedule will be listed at the top. 
* The admin area allows admin users to create new shop schedules with a start time, end time, and rotation type. This is in addition to the 24 hour schedules that the system creates as default when a user visits the shop with no currently active schedule. 
* Each Shop Schedule contains inline shop schedule items, so that they can be easily managed directly from the shop schedule. This means admin can click on a particular schedule and see at a glance which characters will be appearing in that timeframe, remove them if they want, or even add more items.

* Shop Scheduler also has a custom method to show characters allocated to each schedule as a comma separated list, so that admin don'thave to click and open each schedule to find out who will be/was in that shop schedule.
...
* User admin has UserProfile and Usercards inline, so that a User's details, their custom details, and their purchased cards can all be viewed/edited from a central place. 
* Usercards exists in admin separately so all purchases can be viewed in one place sorted by date descending, but its easier for admin to find a particular user's cards by finding that user. 
...
* Audio effects
* A fancier binder
* Ability to reset all cards to their front face in the binder with a button.
...
* Progress bar that shows your binder alignment towards the dark/light through UI visuals, based on quantity of cards purchased and their alignment type.
...
Models include:
* **CharacterCard**: Stores the base characters in the system and their basic information - extended data is kept in a JSON file (legends.json)
...
Enables per-rotation pricing or other metadata without affecting the price stored on the rarity model.
...
* Home Page - no errors  
![Home page accessibility check on Wave](./docs/testing/wave-home.png)

* Shop Page - no errors  
![Shop Page accessibility check on Wave](./docs/testing/wave-shop.png)

* Binder Page - no errors  
![Binder Page accessibility check on Wave](./docs/testing/wave-binder.png)

* Register Page - no errors  
![Register Page accessibility check on Wave](./docs/testing/wave-register.png)

* Login Page - no errors  
![Login Page accessibility check on Wave](./docs/testing/wave-login.png)

* Logout Page - no errors  
![Logout Page accessibility check on Wave](./docs/testing/wave-logout.png)

* Password Reset Page - no errors    
![Password Reset Page accessibility check on Wave](./docs/testing/wave-password.png)

* Password Change Page - no errors    
![Password hange Page accessibility check on Wave](./docs/testing/wave-password-change.png)

* Card Detail Page - no errors  
![Card Detail Page accessibility check on Wave](./docs/testing/wave-card-detail.png)

* Payment Success - no errors  
![Payment Success Page accessibility check on Wave](./docs/testing/wave-payment-success.png)

* Payment Cancel - no errors  
![Payment Cancelled Page accessibility check on Wave](./docs/testing/wave-payment-cancel.png)

* View Profile Page - no errors  
![View Profile page accessibility checkn on Wave](./docs/testing/wave-view-profile.png)

* Edit Profile  
![Edit Profile accessibility check on Wave](./docs/testing/wave-edit-profile.png)

#### **Chrome Lighthouse**
* Home Page(mobile)  
![Home page lighthouse testing on mobile](./docs/testing/homepage-lighthouse-mobile.png)
* Home Page(desktop)  
![Home page lighthouse testing on desktop](./docs/testing/homepage-lighthouse-desktop.png)

* Shop Page(mobile)  
![Shop page lighthouse testing on mobile](./docs/testing/shop-lighthouse-mobile.png)
* Shop Page(desktop)  
![Shop page lighthouse testing on desktop](./docs/testing/shop-lighthouse-desktop.png)

* Binder Page(mobile)  
![Binder page lighthouse testing on mobile](./docs/testing/binder-lighthouse-mobile.png)
* Binder Page(desktop)  
![Binder page lighthouse testing on desktop](./docs/testing/binder-lighthouse-desktop.png)

* Card Detail Page(mobile)  
![Card detail page lighthouse testing on mobile](./docs/testing/card-detail-lighthouse-mobile.png)
* Card Detail Page(desktop)  
![Card detail page lighthouse testing on desltop](./docs/testing/card-detail-lighthouse-desktop.png)

* Payment Success Page(mobile)    
![Payment success page lighthouse testing on mobile](./docs/testing/payment-success-lighthouse-mobile.png)
* Payment Success Page(desktop)    
![Payment success page lighthouse testing on desktop](./docs/testing/payment-success-lighthouse-desktop.png)

* Payment Cancel Page(mobile)    
![Payment cancel page lighthouse testing on mobile](./docs/testing/payment-cancel-lighthouse-mobile.png)
* Payment Cancel Page(desktop)    
![Payment cancel page lighthouse testing on desktop](./docs/testing/payment-cancel-lighthouse-desktop.png)

* View Profile Page(mobile)  
![View profile page lighthouse testing on mobile](./docs/testing/view-profile-lighthouse-mobile.png)
* View Profile Page(desktop)   
![View profile page lighthouse testing on desktop](./docs/testing/view-profile-lighthouse-desktop.png)

* Edit Profile Page(mobile)   
![Edit profile page lighthouse testing on mobile](./docs/testing/edit-profile-lighthouse-mobile.png) 
* Edit Profile Page(desktop)    
![Edit profile page lighthouse testing on mobile](./docs/testing/edit-profile-lighthouse-desktop.png)

* Login Page(mobile)  
  ![Login page lighthouse testing on mobile](./docs/testing/login-lighthouse-mobile.png)
* Login Page(desktop)    
![Login page lighthouse testing on desktop](./docs/testing/login-lighthouse-desktop.png)

* Logout Page(mobile)  
  ![Logout page lighthouse testing on mobile](./docs/testing/logout-lighthouse-mobile.png)
* Logout Page(desktop)    
![Logout page lighthouse testing on desktop](./docs/testing/logout-lighthouse-desktop.png)

* Register Page(mobile)    
![Register page lighthouse testing on mobile](./docs/testing/register-lighthouse-mobile.png)
* Register Page(desktop)    
![Register page lighthouse testing on desktop](./docs/testing/register-lighthouse-desktop.png)

* Password Reset Page(mobile)    
![Password reset page lighthouse testing on mobile](./docs/testing/password-lighthouse-mobile.png)
...
* Password Change Page(mobile)    
![Password change page lighthouse testing on mobile](./docs/testing/password-change-lighthouse-mobile.png)
* Password Change Page(desktop)   
![Password change page lighthouse testing on desktop](./docs/testing/password-change-lighthouse-desktop.png)
...
* Help rendering form fields, while using crispy forms, as readonly. Decided not to use this method ultimately, created the form with disabled fields in the view instead - [stackoverflow](https://stackoverflow.com/questions/21559380/django-crispy-forms-readonly)
...
## **Interesting Bugs**
* The API I wanted to use came with a json file that would allow me to import the characters directly into the database, however there was a lot of extended information that didn't make sense to store in the database without breaching Database Normalisation. I ended up creating a basic CharacterCard model and then stored the json file in core/data/legends.json for the app to open and use the extended data.
