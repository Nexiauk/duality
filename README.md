...
* Not a bug, but something I found interesting to accomplish - creating management commands
    ...
    4. **fix_legend_id** to open the dumped data for the CharacterCard model, iterate through the file and for each item, grab the pk and create a legend_id field that matches the pk number. Then delete the pk and output to a new file. This was in preparation for trashing the CharacterCard data and reimporting.
